        ...
        while True:
            ...
            if first_time == True:
                ...
                if name_enter:
                    ...
                    name_pwd = name_pwd.rstrip().split(
                        ' ')  # The msg format : <name> <password>. This processes it to return an array
                    ...
                    if authenticate(name, pwd):  # If authenticated,i.e, present in local db, then do this
                        ...
                        try:
                            ...
                            del (USERS_LIST[conn])  # Remove the name from the USERS_LIST
                    ...
                    if data == "#!quit":
                        ...
                        try:
                            if data[0] != '@' and data[
                                0] != '^':  # If this msg does not start with @ , then it is broadcast
...
def reg_user(uname, pwd, conn):
    # we will now open /update file to maintain a database
    if uname in open('database.txt').read():
        err = 'Error'
        err = err.encode()
        conn.send(err)
    else:
    ...
    for socket in list_of_sock:
        ...
        if socket in SOCK_LIST:
            try:
                ...
                del (USERS_LIST[SOCK_LIST[socket]])
                del (SOCK_LIST[socket])
        elif socket != sock:  # Redundant, used in the initial terminal implementation
            try:
...
if __name__ == "__main__":
    ...
    sock = socket()
    # Defining server address and port ### Update to wlp6s0 if not connected
    ...
    RECV_BUFFER = 80000000  # Advisable to keep it as an exponent of 2
