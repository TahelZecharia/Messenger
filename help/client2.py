...
import base64  # importing base 64 for multimedia encoding
...
class Application(Tkinter.Tk):
    ...
    def launch_client(self):
        ...
        self.protocol('WM_DELETE_WINDOW', self.client_quit)  ### X button close functionality
        ...
        self.button_frame = ttk.Frame(self.entry_frame)  # For the send and send mutimedia buttons
        ...
        self.chat_entry.pack(side=Tkinter.LEFT, fill=Tkinter.X, expand=True, padx=5)
        self.send_button.grid(row=0, column=0)
        self.browsebutton.grid(row=2, column=0)
        self.button_frame.pack(side=Tkinter.RIGHT)

        self.checks = []
        self.check_label = ttk.Label(self.clients_frame,
                                     width=15,
                                     wraplength=125,
                                     anchor=Tkinter.W,
    ...
    def browse(self):
        self.mmfilename = tkFileDialog.askopenfilename()  # Opens a dialog box for selecting the file and returns  a file path
    ...
    def multimedia_send(self):
        ...
        with open(filename, "rb") as file:  # Read in binary digits
    ...
    def clientchat(self):
        while not self.should_quit:  # If we are not in the 'quit' state then do :
            try:
                data = self.conn.recv(80000000)  # Receive and decode the data
                data = data.decode()
                data = data.rstrip()

                if len(data):  # If there is data
                    # if there is an active user message received then it means a new
                    # user has logged in and we need to update checkbox
                    if data[0] == "!":
                    ...
                    elif data[0] == "^":  # Processing the multimedia message
                ...
                if data == "What is your name?":  # Getting the first message from the server
                    ...
                    try:
                        list_of_active_user = self.conn.recv(
                            4096).decode()  # this will be a string of name separated by spaces
                        list_of_active_user = list_of_active_user.rstrip()  # this will be a string of name separated by spaces
                    except:
                        self.conn.close()
                        self.client_menu()
